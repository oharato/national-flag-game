...
- ✅ **Study.test.ts**: 22/22合格 - 学習モードコンポーネント
...
### 8. Study コンポーネントテスト (`src/views/__tests__/Study.test.ts`) - 22テスト
...
#### 国旗一覧機能
- 国旗一覧が表示されることを確認
- 国旗一覧の国旗をクリックすると上の国旗が変わることを確認

#### クイズ形式切り替え機能
- クイズ形式が選択できることを確認
- 国旗→国名モードでは国旗が表示されることを確認
- 国名→国旗モードでは表に国名と詳細情報が表示されることを確認
- 国旗→国名モードでカードを裏返すと国名と詳細情報が表示されることを確認

## CI/CD統合

### GitHub Actions との統合
テストは GitHub Actions のワークフローに統合されており、以下のタイミングで自動実行されます:

1. **プルリクエスト作成時** (`.github/workflows/test.yml`)
   - `npm ci` で依存関係をインストール
   - `npm test -- --run` でテストを実行
   - `npm run build` でビルド（型チェック含む）

2. **mainブランチへのマージ時** (`.github/workflows/deploy.yml`)
   - testジョブでテストを実行
   - テスト成功後にのみdeployジョブが実行される
   - デプロイ前に品質を保証

これにより、以下が実現されます:
- mainブランチへの問題のあるコードのマージを防止
- 本番環境へのデプロイ前のテスト実行を保証
